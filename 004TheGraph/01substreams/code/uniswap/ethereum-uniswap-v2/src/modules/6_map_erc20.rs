...
use crate::abi::erc20::functions::{Decimals, Name, Symbol, TotalSupply};
use substreams::Hex;
        ...
        for log in transaction.receipt.unwrap().logs {
            ...
            if log.topics.len() > 0 {
                ...
                if topic0_hex.starts_with(TRANSFER_EVENT_SIGNATURE) {
                    ...
                    match substreams_ethereum::rpc::RpcBatch::decode::<_, TotalSupply>(
                        &responses[3],
                    ) {
...
fn save_erc20(
    ...
    changes
        .push_change_composite("ethereum_block_erc20", composite_key, 1, Operation::Create)
